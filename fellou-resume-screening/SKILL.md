...
For frontend, full-stack, mobile, Design Engineer, Agent platform, coding-tool, or AI engineering batches, also read [references/positive-resume-patterns.md](references/positive-resume-patterns.md). Use it to identify high-priority "林明型" candidates: people who combine AI coding/Agent systems, engineering ownership, productized tools, and evaluation or delivery loops.
...
10. For frontend/full-stack/R&D batches, explicitly mark `林明型接近度` when evidence supports it. High closeness means the candidate has built coding IDEs, Agent systems, design tools, canvas/editor systems, compiler/build tooling, AI engineering infrastructure, or AI-productized developer tools with real usage or delivery evidence.
11. Return candidate, best-fit role, score, three strongest evidence points, gaps, verification questions, and recommendation.
12. Apply bands: `80–100 优先面试`, `70–79 人工复核/人才池`, `0–69 暂不推进 / 不展示`. Scores at or above 70 qualify for report display and human review, not automatic hiring. Candidates below 70 should be recorded only in the minimal deduplication ledger, not included in report details or recommendation lists.
13. After recruiter corrections, new JD inputs, interview feedback, or repeated screening patterns, distill the lesson into this local Skill rather than treating it as a one-off note. Append reusable corrections to [references/calibration-log.md](references/calibration-log.md), update the profile if needed, and validate on a fresh non-overlapping batch.
